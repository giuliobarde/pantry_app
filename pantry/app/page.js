...
const Page = () => {
  ...
  // Function to clean the recipe output
  const cleanRecipeOutput = (text) => {
      // Split the text into lines
      const lines = text.split('\n');

      // Filter out lines that are not part of the recipe
      const recipeLines = lines.filter(line => {
          // Add conditions to filter out non-recipe text
          // This can include removing empty lines, lines with certain keywords, etc.
